...
const StepCard: React.FC<Step & { delay: number }> = ({number, icon, title, description, delay}) => {
  ...
  return (
    ...
    >
      <div
        className="text-5xl font-bold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950 border-2 border-amber-400 dark:border-amber-700 rounded-full w-20 h-20 flex items-center justify-center mb-6 font-serif shadow-sm leading-none pb-3">
        {number}
      </div>
      <div
